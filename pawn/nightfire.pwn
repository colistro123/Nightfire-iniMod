...
new Sounds[MAX_SOUNDS][ESoundsInfo] = {
{"space/SFV_one.wav", 1},
{"space/SFV_two.wav", 2},
...
public OnServerChangeMap()
{
	...
	PrecacheModel("models/c4_explosives.mdl");
}
public OnMapLoaded()
{
...
public OnPluginInit()
{
	print("The gamemode \"plugin1.amx\" has been loaded successfully!");
	new string[128];
...
public OnPluginExit()
{
	print("The gamemode \"plugin1.amx\" has been unloaded successfully!");
	exit(0);
	return 1;
}
...
public OnClientCommandText(playerid, cmdname[]) {
	...
	} else if(!strcmp(cmd[0], "/kick")) { //Check if we got the right cmd
		new user;
		//Params
		if(strlen(cmd[1]) < 1 || strlen(cmd[2]) < 1) {
			TriggerHudMsg(playerid, "Wrong Syntax: /kick <id> <reason>", 5, 1);
			return 1;
		}
		split(cmdname, cmd, ' ');
  		user = strval(cmd[1]);
		strformat(smsg, sizeof(smsg), false, "%s", cmd[2]);
		KickClient(user, smsg);
    ...
    } else if(!strcmp(cmd[0], "/screenfade")) {
		UTIL_ScreenFade(playerid, 0, 0, 0, 255, 5, 10, 2);
...
thinkKills(attacker, receiver) {
	...
	new sprIndex = PrecacheModel("sprites/laser_beam.spz");
	UTIL_CreateBeamEntsPlayers(attacker, receiver, sprIndex, 0, 0, 15, 10, 0, random(255), random(255), random(255), 255, 0, MSG_BROADCAST);
}
getKillsLeader() {
