...
typedef struct engine_api_s
{
	...
	// Functions
	int		( *CvarRegister )				( cvar_reg pCvar );
	bool	( *WrapperActivateClientCommand ) (const char *cmd);
	int		( *RegClientCommand )			( const char *cmd);
	void	( *ActivateServerCommand )		( int *cmd );
	void	( *ActivateClientCommand )		( int *cmd );
	int		( *UnknownEngineFunc1 )			( int one, int two, int three, int four );
	int		( *UnknownEngineFunc2 )			( int one );
	...
	//void	( *CommandProcessUnknown )		( const char *str1, const char *str2, const char *str3 );
	void	( *Host_ReadConfiguration )		( void );
	void	( *UnknownEngineFunc3 )			( unsigned char one );
	//int		( *Cmd_Argc )					( void );
	//char	*( *Cmd_Args )					( void );
	...
	bool	( *Game_Init )					( char *lpCmdLine, unsigned char *pMem, int iSize, struct exefuncs_s *pef, int *unknown1, void *, int, int *unknown2 );
	void	( *UnknownEngineFunc4 )			( int one, int two );
	...
	//void	( *SetMessagePumpDisableMode )	( unsigned char bMode );
	//void	( *SNDDMA_Shutdown )			( void );
	//void	( *Snd_AcquireBuffer )			( void );
	//void	( *Snd_ReleaseBuffer )			( void );
	//void	( *StoreProfile )				( void );
	double	( *Sys_FloatTime )				( void );
	void	( *UnknownEngineFunc5 )			( void );
	void	( *UnknownEngineFunc6 )			( void );
	unsigned char* ( *LoadFile )			( const char *path, int *pLength );
	unsigned char* ( *LOAD_FILE_FOR_ME )	( const char *path, int *pLength );
	void	( *FREE_FILE )					( unsigned char *buffer );
	int		( *COM_ExpandFileName )			( char *filename );
	void	( *UnknownEngineFunc7 )			( int one );
	/*
